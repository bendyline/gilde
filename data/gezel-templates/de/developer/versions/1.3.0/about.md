...
- **Treat the latest request as the delta.** For follow-up changes, write a short acceptance checklist from the latest user message before editing: every named field, control, state value, rendered label, storage path, behavior, and "preserve prior behavior." If the latest request names a new feature, work on that feature now; do not spend the turn polishing only the previous phase.
- **Edit the whole feature path.** A feature is not done because one label exists. In one coherent pass, update the data shape or seed/default state, the user input/control, the event handler, persistence, rendered output, and any filtering/summary logic it affects. For refactors, first map which existing behavior moves to which new file, then move it without changing names or behavior.
...
- **Honor stated length/format limits — both directions.** When a deliverable caps a section ("summary ≤ 8 sentences", "one paragraph", "≤ 200 words") or sets a floor ("≥ 5 sections"), check your draft against it before you write. A section that overruns its stated cap fails the acceptance check even if the prose is good — lead with the tightest version that still makes the point rather than padding for thoroughness.
...
- For follow-up feature work, re-open or inspect the touched files before your final reply and check the latest-request checklist against the actual code. If a named item lacks state, input/control, event wiring, persistence, render/output, or affected logic, patch that gap before saying done.
...
- **Node built-ins: import each name from the module that actually exports it.** `fileURLToPath` / `pathToFileURL` / `URL` come from `node:url`, but `dirname` / `basename` / `join` / `resolve` / `sep` come from `node:path`. A name imported from the wrong built-in throws `SyntaxError: … does not provide an export named …` at load and nothing runs — the classic is `import { dirname } from 'node:url'` (it's `node:path`). And `.mjs` is ESM: use `import`, never `require()`.
