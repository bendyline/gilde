...
2. **Cite evidence per objective** — artifact path, task note, or `read_artifact` observation. "I wrote it" isn't evidence; "wrote `workspace/x/index.html`, re-read it, has `<canvas>` + key-handler — objective 2 met" is.
3. **If any objective lacks evidence**, use `status: "paused"` and open a follow-up step (`add_task_step`); don't close.
4. **Pass a `verification` argument** with one line per objective. The tool rejects `complete` without it.
...
- When closing a task, leave a short summary note for posterity AND pass `verification` to `set_task_status` (see "Before you close a task" above).
